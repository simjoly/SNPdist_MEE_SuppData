...
|                                              |
|                                              |
|   (c) Copyright 2005-2013 by Simon Joly      |
|                                              |
\**********************************************/
...
Instructions
    ...
    Usage:
      pofad [-i] [file_containning_the_organims]
            ...
            [-d] [file_containning_the_name of datasets]
            [-m] [distane method: 0=genpofad (default), 1=matchstates, 2=mrca, 3=pofad, 4=Output consensus sequences]
            [-c] [sequence_file_for_consensus sequence]
            [-g] [Gives gap handling: 0=missing, 1=fifth state]
            [-o] [name_for_the_output_file]            
            [-v] [sets the program in verbose mode]

    Options: -v  Set the program in verbose mode
...
int main(int argc, char **argv)
{
	...
	if (!info_data.ReturnIsBatchFile())  /* If running batch file, don't do this */
	    ...
	    for (;;)
		    ...
		    else if (info_data.ReturnDistanceMethod() == 5) cout <<"Phylogenetic Bray-Curtis"<< endl;
	        ...
	        switch (toupper(character))
	            ...
	            case 'A':
	                for (;;)
	                    ...
	                    cout << " (g)   pofadX                                    " << endl;
	                    cout << " (h)   MIN"                                        << endl;
                        cout << " (i)   Nei's genetic distance"                     << endl;
	                    ...
	                    switch (toupper(selection))
	                        {
	                        case 'A':
	                            info_data.SetDistanceMethod(0);
	                            break;
	                        case 'B':
	                            info_data.SetDistanceMethod(1);
	                            break;
	                        case 'C':
	                            info_data.SetDistanceMethod(2);
	                            break;
	                        case 'D':
	                            info_data.SetDistanceMethod(3);
	                            break;
	                        case 'E':
	                            info_data.SetDistanceMethod(4);
	                            break;
	                        case 'F':
	                            info_data.SetDistanceMethod(5);
	                            break;
	                        case 'G':
	                            info_data.SetDistanceMethod(7);
	                            break;
	                        case 'H':
	                            info_data.SetDistanceMethod(8);
	                            break;
                            case 'I':
                                info_data.SetDistanceMethod(9);
                                break;
	                        case 'J':
	                        ...
	                        default:
	                            ...
	                            cout << " Enter a valid choice: a, b, c, d, e, f, g, h, i, or j  " << endl;
	                            continue;
	                        }
	                    break;
	                    }
...
//TODO: change the next if for a while?
    if ( !info_data.ReturnIsOrganismsFile() )  //If no organism file was entered as arguments in batchfile mode
        {
...
//TODO:prompt for sequence file if not entered and if using batch mode
    ...
    {                                                     //third is for the column of the matrix.
        dist_matrix[i] = new double* [org_data.ReturnNOrg()];   
    ...
    for (i=0; i < info_data.ReturnDatasetsNumber(); i++)                                  //to the datasets, the 2nd is for the matrix' rows whereas the 
    {                                                         //third is for the column of the matrix.
        stand_dist_matrix[i] = new double* [org_data.ReturnNOrg()];   
        for (j=0; j < org_data.ReturnNOrg(); j++)
        {
    ...
    else if (info_data.ReturnDistanceMethod() == 4) logfile <<"Consensus sequences"<< endl;
...
void consensus(string FileName)
{
    ...
    for (i=0; i<org_data.ReturnNOrg(); i++)
        ...
        //The following is in case this organisms is not represented in the matrix
        string astring, astring_a;
        astring = To_Uppercase(org_data.ReturnOrganism(i));
        astring_a = To_Uppercase(org_data.ReturnOrganism(i)) + "_A";
        if ( (get_position_nexus(astring) == -1) && (get_position_nexus(astring_a) == -1) ) continue;
...
void read_nexus_matrix(double ***dist_matrix, double ***stand_dist_matrix, int set)
{
    ...
    if ((info_data.ReturnDistanceMethod() == 0) || (info_data.ReturnDistanceMethod() == 1) || (info_data.ReturnDistanceMethod() == 2)) {
	    org_data.GetNChar(nexus_data.ReturnNChars());
	    org_data.MissingChar(nexus_data.ReturnMissingChar());
	    org_data.GetGapChar(nexus_data.ReturnGapChar());
	    org_data.GetDatatype(nexus_data.ReturnDatatype());
	    ...
	    if (To_Uppercase(nexus_data.ReturnDatatype()) != "DNA") {
            cerr << "Only DNA characters are allowed";
            exit(1);
        }
		...
		} /* end if ((info_data.ReturnDistanceMethod() != 3) || (info_data.ReturnDistanceMethod() != 4)) */
    ...
    else if (info_data.ReturnDistanceMethod() == 8) //MIN distance of Göker and Grimm
    	{
		if (!nexus_data.ReturnIsDistanceMatrix()) {
			nexus_data.InitMatrix();
			nexus_data.ComputeDistanceMatrix();
    ...
    else if ( info_data.ReturnDistanceMethod() != 4 ) {   /* Either genpofad, matchstates, or mrca */
	    for (i=0; i < org_data.ReturnNOrg(); i++) {
	        for (j=0; j <= i; j++) {
	...
	else if (info_data.ReturnDistanceMethod() == 4 || info_data.ReturnDistanceMethod() == 9) {   /* FRQ */
	    for (i=0; i < org_data.ReturnNOrg(); i++) {
	        for (j=0; j <= i; j++) {
				raw_distance = org_data.ReturnDist(i, j);
				/*
				Jukes and Cantor correction not implemented for the FRQ distance
				if (info_data.ReturnIsJukesCantor()) { 		// Jukes and Cantor distance
					if (raw_distance == -999) {  // No data
		            	dist_matrix[set][i][j] = raw_distance;
		            	dist_matrix[set][j][i] = raw_distance;						
				...
				*/
//				if (!info_data.ReturnIsJukesCantor()){ /* Hamming distance */	
            	dist_matrix[set][i][j] = raw_distance;
            	dist_matrix[set][j][i] = raw_distance;
//	            			}
	            }
...
The function get_distance returns a distance between two organisms from allelic data of these two individuals

Variables:
	- the vector containning the organisms' names
	- the vector containning the haplotypes' names
	- the matrix containning the pairwise haplotype distance
	- the position in the vector of organisms of the first organism
	- the position in the vector of organisms of the second organism
...
double get_nexus_distance(int m, int n)
{
    ...
    string string_m, string_ma, string_mb, string_n, string_na, string_nb;
    ...
    string_m = To_Uppercase(org_data.ReturnOrganism(m));
    string_ma = To_Uppercase(org_data.ReturnOrganism(m)) + "_A";
    string_mb = To_Uppercase(org_data.ReturnOrganism(m)) + "_B";
    string_n = To_Uppercase(org_data.ReturnOrganism(n));
    string_na = To_Uppercase(org_data.ReturnOrganism(n)) + "_A";
    string_nb = To_Uppercase(org_data.ReturnOrganism(n)) + "_B";
    //If one of the organisms is absent from the dataset, return the flag "-999".
    if ( ( (get_position_nexus(string_m) == -1) && (get_position_nexus(string_ma) == -1) ) || ( (get_position_nexus(string_n) == -1) && (get_position_nexus(string_na) == -1) ) )
    {
    return (-999);
    }
    //If both organisms compared have only a single haplotype
    if ( (get_position_nexus(string_ma) == -1) && (get_position_nexus(string_na) == -1) )
    {
        distance = nexus_data.ReturnDist(get_position_nexus(string_m), get_position_nexus(string_n));

        #ifdef DEBUG
            cout << "d(" << org_data.ReturnOrganism(m) << "," << org_data.ReturnOrganism(n) << ") = " << distance << endl;
        #endif

        return distance;
    }
    //If the first organism (m) has only one allele whereas the second (n) has two alleles
    else if ( (get_position_nexus(string_ma) == -1) && !(get_position_nexus(string_na) == -1) )
    {
        distance = ( (nexus_data.ReturnDist(get_position_nexus(string_m), get_position_nexus(string_na)) ) + (nexus_data.ReturnDist(get_position_nexus(string_m), get_position_nexus(string_nb)) ) ) / 2;
    ...
    //If the first organism (m) has two alleles whereas the second (n) has one allele
    else if ( !(get_position_nexus(string_ma) == -1) && (get_position_nexus(string_na) == -1) )
    {
        distance = ( (nexus_data.ReturnDist(get_position_nexus(string_ma), get_position_nexus(string_n)) ) + (nexus_data.ReturnDist(get_position_nexus(string_mb), get_position_nexus(string_n)) ) ) / 2;
    ...
    //If both organisms have two alleles
    else
    {
        double distance1;
        double distance2;
        ...
        #ifdef DEBUG
            cout << " -> both have 2 alleles" << endl;
            cout << "Alleles: " << string_ma << '\t' << string_mb << '\t' << string_na << '\t' << string_nb << endl;

            cout << "d(" << string_ma << "," << string_na << ") = " << nexus_data.ReturnDist(get_position_nexus(string_ma), get_position_nexus(string_na)) << endl;
            cout << "d(" << string_mb << "," << string_nb << ") = " << nexus_data.ReturnDist(get_position_nexus(string_mb), get_position_nexus(string_nb)) << endl;
            cout << "d(" << string_ma << "," << string_nb << ") = " << nexus_data.ReturnDist(get_position_nexus(string_ma), get_position_nexus(string_nb)) << endl;
            cout << "d(" << string_mb << "," << string_na << ") = " << nexus_data.ReturnDist(get_position_nexus(string_mb), get_position_nexus(string_na)) << endl;
        #endif

        distance1 = ( nexus_data.ReturnDist(get_position_nexus(string_ma), get_position_nexus(string_na)) ) + ( nexus_data.ReturnDist(get_position_nexus(string_mb), get_position_nexus(string_nb)) );
        distance2 = ( nexus_data.ReturnDist(get_position_nexus(string_ma), get_position_nexus(string_nb)) ) + ( nexus_data.ReturnDist(get_position_nexus(string_mb), get_position_nexus(string_na)) );
...
void output_file(double **final_matrix, double **final_stand_matrix)
{
    ...
    if (info_data.ReturnDistanceMethod() == 1) outfile << "[!* Distance matrix generated using the matchstates algorithm        *]" << endl;
    if (info_data.ReturnDistanceMethod() == 2) outfile << "[!* Distance matrix generated using the mrca algorithm               *]" << endl;
    if (info_data.ReturnDistanceMethod() == 3) outfile << "[!* Distance matrix generated using the original POFAD algorithm     *]" << endl;
        ...
        outfile << "[!*        -> D. Bryant, S. Joly and P.J. Lockhart. Unpublished data *]" << endl;
        outfile << "[!*                                                                  *]" << endl;
