	...
	if (method == "2ISP")
    {
	    for (i=0;i<NOrg;i++)
        {
	        ...
	        for (j=0;j<=i;j++)
            {
                ...
                {
	                dist_matrix[i][j] = organismsdata::getdistance(i,j,"2ISP");
	                dist_matrix[j][i] = dist_matrix[i][j];
                }
            }
        }
		...
		if (info_data.ReturnIsIgnoreMissingData()) {
			...
			for (x=0;x<NOrg;x++) {
				...
				for (y=0;y<NOrg;y++) {
					...
					dist_matrix[x][y] = distance/numbercomparisons; //SJ: modified for recomb analysis...
					dist_matrix[y][x] = dist_matrix[x][y];
                }			
				dist_matrix[x][x] = 0.;
            }
...
void organismsdata::GetSimChar(int a_number) {
    this->numbercomparisons = a_number;
}
...
//    double numbercomparisons = 0;
    ...
    for (k=0; k < NChar; k++)
    {
        ...
        if (method == "2ISP") totaldistance += organismsdata::get2ISPdist(char_i,char_j);
        //numbercomparisons += 1;
    }
...
double organismsdata::get2ISPdist(string firstchar, string secondchar) {
    ...
    if ( (PositionOfFirstChar > 16) || (PositionOfSecondChar > 16)) {
		cerr << "Method 2ISP does not allow the inclusion of gap characters in polymorphic bases. Exiting..." << endl;
        exit(1);        
    }
